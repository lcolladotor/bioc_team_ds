...
In `OneDrive` ^[In the past we used `Dropbox`, but because `OneDrive` is free through JHU, we switched to it.], use a directory like `submission_01` for the first submission as there will be multiple rounds for any paper. Follow this general structure for the publication files:
...
* Use *italics* for the software names. This context will again help users differentiate software names from actual English words. Think of *recount* versus recount. *recount* is referring to an actual software, whereas [recount refers to the English verb](https://www.merriam-webster.com/dictionary/recount).
* Show the version number of the software ^[For software with no version numbers but that is available from GitHub, use the SHA from the Git commit identifying the exact version you used. For example, "_SPEAQeasy v51ffb22_" for [this specific version of SPEAQeasy](https://github.com/LieberInstitute/SPEAQeasy/commit/51ffb22f6492e2a9a081e33316fa225c3ea1e530).] such as "_recount3_ v1.12.0". Use just the `v` prefix instead of typing `version`, particularly when word count limits are in play.
...
* Figure captions should follow a syntax like this:
  ...
  * Note how in the description of **Figure1B** we call back to the (A) panel with regular font, whereas to describe the (**A**) panel we did use a bold font for the letter A. This use of the bold font for the first mention (aka description) of a panel helps readers visually find where the description of each panel starts. Here are some incorrect formats:
    ...
    * **Figure 1: Figure title in sentence case.** Description of the (A) panel (**A**). Description of the (B) panel, which could refer back to the (A) panel without using bold. But then has more information (**B**). [Aka](https://www.merriam-webster.com/dictionary/aka), we no longer know where the description for (B) starts.
...
* Supplemental figures or tables should refer back to a main figure and/or table at the end of the descriptions.
  ...
  * Using this "Related to ..." syntax will help users easily build a mental network of how the different display items are related to each other.
...
* `habenulaPilot` where I shared it on [Twitter/X](https://twitter.com/lcolladotor/status/1765872084083781840), [Bluesky](https://bsky.app/profile/lcolladotor.bsky.social/post/3kn5bagqqkc26), and [LinkedIn](https://www.linkedin.com/posts/lcollado_transcriptomic-analysis-of-the-human-habenula-activity-7171640139167858688-iiPr?utm_source=share&utm_medium=member_desktop) to reach different co-authors (tag them) and audiences.
...
When in doubt, sign up for a DSgs with those in the team who have promoted our work in the past! Just have your draft message(s) ready.
