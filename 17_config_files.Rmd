...
- macOS: [Cyberduck](https://cyberduck.io/), [iTerm2](https://www.iterm2.com/), [Alfred](https://www.alfredapp.com/), [DockDoor](https://dockdoor.net/)
...
remotes::install_cran("xaringanExtra")
...
Edit it with `usethis::edit_git_config()`. Note that this will configure `git` to use `Positron` as your editor for commit messages.
...
[core]
	ignorecase = true
	editor = positron --wait

```

On my laptop the end is a little different given that I do use a `~/.gitignore_global` file there.
...
In the following set of videos, the set-up process for coding at JHPCE (and
locally) is documented. These videos and notes walk through setting up SSH
keys, git, R, as well as RStudio + either WinSCP or CyberDuck. In the newer
set-up, RStudio + WinSCP/CyberDuck can replaced with Positron, a single
fully integrated code editor (with additional AI features). However, the
original video/notes still include relevant set-up details necessary even with
the new set-up.

For MacOS check this video and [companion notes](https://gist.github.com/lcolladotor/3b1063db30a206835ee7e086c8f037ee):
...
After setting up the appropriate SSH keys, the Positron IDE can be used for coding at JHPCE. In contrast with the previously recommended RStudio set-up, Positron can directly see the remote filesystem at JHPCE, plot interactively, and leverage additional AI features (note a number of coding set-ups are possible at JHPCE, including use of the RStudio web app which also allows seamless integration with the remote filesystem, but we see Positron as the best all-around option).

Check out [these notes](https://docs.google.com/document/d/1lGbZfl-FjrzQXnVsJKIEMTN8ecnFNzE0M0zKFNnVCCA/edit?usp=sharing)
as well as the instructional video:

<iframe width="560" height="315" src="https://www.youtube.com/embed/CnmaJG8VPp8?si=4jke5ysJVyNarcSL" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
alias dcs07="cd /dcs07/lieber/lcolladotor"
