...
Things to check before someone leaves, to make sure we understand the code and files in the project.
...
* Do we have access to the files on JHPCE?
  * Do we have write access to these files?
* Is there a clean `git status` output?
* Are the files organized as in https://lcolladotor.github.io/bioc_team_ds/organizing-your-work.html#.YqrlZ-xBya0?
...
* Is there `sessioninfo::session_info()` output on the log files or commented on the R scripts? This is useful for reproducibility and for writing the methods sections of papers.
  * Check if any of the packages were installed from GitHub. If so, ask why GitHub versions were needed. For example, https://github.com/immunogenomics/harmony/issues/145. Note this on the `README.md` or as a comment on the top of the script.
* Are the scripts linear? That is, can they be re-run from line 1 till the end linearly?
  * Ideally: are there companion shell scripts for the R scripts? For example, https://github.com/LieberInstitute/Visium_IF_AD/blob/master/code/run_all_post_spaceranger.sh.
